...
class SocketExample(object):
     ...
     def start_transformers_workers(self):     
          ...
          self.transformer_sender1 = context.socket(zmq.PUSH)
          self.transformer_sender1.setsockopt(zmq.LINGER, 0)
          self.transformer_sender1.bind("tcp://*:5557")
   
          self.transformer_sender2 = context.socket(zmq.PUSH)
          self.transformer_sender2.setsockopt(zmq.LINGER, 0)
          self.transformer_sender2.bind("tcp://*:5558")
      
          self.transformer_sender3 = context.socket(zmq.PUSH)
          self.transformer_sender3.setsockopt(zmq.LINGER, 0)
          self.transformer_sender3.bind("tcp://*:5559")
          
          
          self.transformer_recv1 = context.socket(zmq.PULL)
          self.transformer_recv1.connect("tcp://localhost:5557")
   
          self.transformer_recv2 = context.socket(zmq.PULL)
          self.transformer_recv2.connect("tcp://localhost:5558")
      
          self.transformer_recv3 = context.socket(zmq.PULL)
          self.transformer_recv3.connect("tcp://localhost:5559")     
     ...
     def start_loader_workers(self):       
          ...
          self.loader_sender1 = context.socket(zmq.PUSH)
          self.loader_sender1.setsockopt(zmq.LINGER, 0)
          self.loader_sender1.bind("tcp://*:7000")
   
          self.loader_sender2 = context.socket(zmq.PUSH)
          self.loader_sender2.setsockopt(zmq.LINGER, 0)
          self.loader_sender2.bind("tcp://*:7001")
      
          self.loader_sender3 = context.socket(zmq.PUSH)
          self.loader_sender3.setsockopt(zmq.LINGER, 0)
          self.loader_sender3.bind("tcp://*:7002")
          
          
          
          
          self.loader_recv1 = context.socket(zmq.PULL)
          self.loader_recv1.connect("tcp://localhost:7000")
   
          self.loader_recv2 = context.socket(zmq.PULL)
          self.loader_recv2.connect("tcp://localhost:7001")
      
          self.loader_recv3 = context.socket(zmq.PULL)
          self.loader_recv3.connect("tcp://localhost:7002")            
          
  
          self.loader_senders=[self.loader_sender1,self.loader_sender2,self.loader_sender3]
     ...
     def start_workers(self):
          ...
          self.senders=[self.transformer_sender1,self.transformer_sender2,self.transformer_sender3] 
          
          self.poller = zmq.Poller()    
          self.poller.register(self.transformer_recv1, zmq.POLLIN)           
          self.poller.register(self.transformer_recv2, zmq.POLLIN)
          self.poller.register(self.transformer_recv3, zmq.POLLIN)
          
          self.loader_poller = zmq.Poller()
          
          self.loader_poller.register(self.loader_recv1, zmq.POLLIN)   
          self.loader_poller.register(self.loader_recv2, zmq.POLLIN)
          self.loader_poller.register(self.loader_recv3, zmq.POLLIN)
     ...
     def transformer_level(self):        
          ...
          while listening:         
               ...
               if socks.get(self.transformer_recv1) == zmq.POLLIN:
                    msg =simplejson.loads(self.transformer_recv1.recv())    
                    #print msg
                    self.records.append(msg)   
                    self.loader_push(msg)
           
               if socks.get(self.transformer_recv2) == zmq.POLLIN:
                    msg =simplejson.loads(self.transformer_recv2.recv())  
                    #print msg
                    self.records.append(msg)
                    self.loader_push(msg)
                  
               if socks.get(self.transformer_recv3) == zmq.POLLIN:
